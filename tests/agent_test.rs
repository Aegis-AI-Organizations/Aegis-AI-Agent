...
#[tokio::test]
#[serial]
async fn test_load_or_register_agent_re_registers_if_stored_secret_is_stale() {
    let mut server = mockito::Server::new_async().await;
    let temp_dir = tempfile::tempdir().unwrap();
    let config_path = temp_dir.path().join(".agent_secret_test");
    let config = AgentConfig {
        agent_id: "agent-stale".to_string(),
        agent_secret: "secret-stale".to_string(),
    };

    let _heartbeat = server
        .mock("POST", "/api/agents/agent-stale/status")
        .with_status(403)
        .create_async()
        .await;
    ...
    let _register = server
        ...
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(r#"{"agent_id": "agent-fresh", "agent_secret": "secret-fresh"}"#)
        .create_async()
        .await;
    ...
    let loaded = load_or_register_agent_exposed().await.unwrap();
    assert_eq!(loaded.agent_id, "agent-fresh");
    assert_eq!(loaded.agent_secret, "secret-fresh");
